...
//        Mockito.when(mockAdminService.returnInformationForMenuBadges()).thenReturn(informationForMenuBadges);
//        mvc = MockMvcBuilders
...
//        Mockito.when(mockAdminService.isAdminHasDefaultPassword()).thenReturn(false);
...
//        Mockito.when(mockAdminService.isAdminHasDefaultPassword()).thenReturn(true);
//        mvc.perform(get("/admin/").contentType(MediaType.APPLICATION_JSON))
...
//        Mockito.when(mockAdminService.returnAllPersonsWithAccounts()).thenReturn(personMitAccountList);
